...
See [Global Payment Options](https://confluence.nike.com/pages/viewpage.action?pageId=162870810) for a list of supported payment types by shipping and billing country.
...
For v2 Checkout, use the [Get Payment Options v2](https://console.platforms.nike.com/developer/docs/projects/Payment%20Options?tab=api#get-payment-options-post) endpoint:

`GET https://api.nike.com/payment/options/v2`

**v3 Checkout**

For v3 Checkout, use the [Get Payment Options v3](https://console.platforms.nike.com/developer/docs/projects/Payment%20Options%20v3%20(Source%20Aware)?tab=api) endpoint:
...
You can get the list of valid billing countries based on the consumer's shipping country using the [Get Billing Countries for Shipping Country](https://console.platforms.nike.com/developer/docs/projects/Payment%20Options?tab=api#get-billing-countries-for-shipping-country-get) endpoint. Your experience can use this list to display only valid billing countries to the consumer and perform shipping/billing country validation as early in the purchase process as possible.

Note that the results are unsorted.

Listed below is a sample [Get Billing Countries for Shipping Country](https://console.platforms.nike.com/developer/docs/projects/Payment%20Options?tab=api#get-billing-countries-for-shipping-country-get) GET request URI. It is not JWT-restricted.
...
Use the [Validate Payments](https://console.platforms.nike.com/developer/docs/projects/Payment%20Options?tab=api#payment-options-service-post-1) endpoint to check each payment on the Checkout selected by the consumer is valid for the shipping country. The POST request body should include a client-generated UUID, payment type and billing country for each Checkout payment, and the shipping country.

Listed below is a sample [Validate Payments](https://console.platforms.nike.com/developer/docs/projects/Payment%20Options?tab=api#payment-options-service-post-1) POST request URI. It is not JWT-restricted.
...
Use the [Add Stored Payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-add-stored-payment-post) endpoint to save a consumer's payment for future use. For PayPal stored payments, see [Start a PayPal Billing Agreement](#start-a-paypal-billing-agreement). The request body varies depending upon the payment type and whether the endpoint is called pre-authorization or post-authorization.

The typical flow for storing a new credit card **pre-authorization** for a consumer is:

1. Call the [Add Credit Card with CVV](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-add-credit-card-with-cvv-get) endpoint of the Credit Card Payment service to securely transmit credit card information via iFrame to the PCI-certified Credit Card Payment service, passing a client-generated UUID as the `creditCardInfoId`.

2. Call [Add Stored Payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-add-stored-payment-post), passing the same `creditCardInfoId` from **Step 1** to look up the credit card information from short term storage and save it to long term storage.

3. Call [Get Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-payments-by-upmid-post) to get all stored payments including the one just saved to confirm that the credit card was securely stored. Credit Card account numbers are masked in the response.

Listed below is the [Add Stored Payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-add-stored-payment-post) POST request URI. This endpoint requires a service-to-service JWT in the X-Nike-Authorization header.
...
Use the [Modify Credit Card Stored Payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-modify-credit-card-stored-payment-put) endpoint to update a stored credit card's expiration month, expiration year, billing address and default payment type flag, or to update a gift certificate default payment type flag. If you only need to update the default payment type flag of either a credit card or gift certificate stored payment, see [Modify the Default Stored Payment](#modify-the-default-stored-payment).

The typical flow for modifying a stored credit card is:

1. [Get Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-payments-by-upmid-post) to get all of a consumer's stored payments. Credit Card account numbers are masked in the response.

2. Call [Modify Credit Card Stored Payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-modify-credit-card-stored-payment-put) . Get the `payment_id` path parameter from the appropriate `paymentId` field that was returned in the response of **Step 1**.

3. [Get Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-payments-by-upmid-post) to get all stored payments including the one just modified to confirm that the credit card was updated. Credit Card account numbers are masked in the response.

Listed below is the [Modify Credit Card Stored Payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-modify-credit-card-stored-payment-put) PUT request URI. This endpoint requires a service-to-service JWT in the X-Nike-Authorization header.
...
Use the [Modify the Default Stored Payment'](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-modify-default-stored-payment-put) endpoint to update the default stored payment. This endpoint removes the default flag on the current default stored payment and adds the flag to the stored payment matching the `payment_id` path parameter. An experience can use the default payment type to pre-select a payment method in the shopping flow. If you need to update a stored credit card's expiration month, expiration year, billing address and default payment type flag, see [Modify a credit card stored payment](#modify-a-credit-card-stored-payment).

The typical flow for modifying the default stored payment is:

1. [Get Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-payments-by-upmid-post) to get all of a consumer's stored payments. Credit Card account numbers are masked in the response.

2. Call [Modify Default Stored Payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-modify-default-stored-payment-put) passing the `payment_id` path parameter using the appropriate `paymentId` field returned in the response of **Step 1**.

3. [Get Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-payments-by-upmid-post) to get all stored payments including the one just modified to confirm that the default credit card was changed. Credit Card account numbers are masked in the response.

Listed below is a [Modify Default Stored Payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-modify-default-stored-payment-put) PUT request URI. The endpoint is not JWT-restricted.
...
Use the [Delete Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-delete-stored-payments-by-upmid-delete) to delete all of a consumer's stored payments. If you want to delete just one of a consumer's stored payments, see [Delete Stored Payment by ID](#delete-stored-payment-by-id).

The typical flow for deleting all stored payments for a consumer is:

1. [Get Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-payments-by-upmid-post) to get all of a consumer's stored payments. Credit Card account numbers are masked in the response.

2. Call [Delete Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-delete-stored-payments-by-upmid-delete) to delete all of a consumer's stored payments.

3. [Get Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-payments-by-upmid-post) to verify that no stored payments are returned.

Listed below is a sample [Delete Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-delete-stored-payments-by-upmid-delete) DELETE request URI. **This endpoint is JWT-restricted.**
...
Use the [Delete Stored Payment by ID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-modify-credit-card-stored-payment-delete) endpoint to delete a stored payment by payment ID. For example, this endpoint would be called when consumers delete a stored payment when managing their payment information in the experience.

The typical flow for deleting a stored payment for a consumer is:

1. [Get Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-payments-by-upmid-post) to get all of a consumer's stored payments. Credit Card account numbers are masked in the response.

2. Call [Delete Stored Payment by ID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-modify-credit-card-stored-payment-delete) to delete a consumer's stored payment for the given `payment_id`.

3. [Get Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-payments-by-upmid-post) to verify that no stored payments are returned.

Listed below is a [Delete Stored Payment by ID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-modify-credit-card-stored-payment-delete) DELETE request URI. This endpoint requires a service-to-service JWT in the X-Nike-Authorization header.
...
Use the [Get Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-payments-by-upmid-post) endpoint to list all of a consumer's stored payments. Account numbers are masked in the response. If you are a retail client, use the [Get Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-payments-by-upmid-get) (Retail) endpoint instead.
...
Listed below is a sample [Get Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-payments-by-upmid-post) POST URI request. This endpoint requires a service-to-service JWT in the X-Nike-Authorization header.
...
Use the [Get Stored Payments by UPMID Retail](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-payments-by-upmid-get) if you are a retail client to list all stored payments for a consumer. If you are not a retail client, use the [Get Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-payments-by-upmid-post) endpoint instead.
...
Use the [Get Stored Gift Certificate by ID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-gift-certificates-by-id-get) endpoint to list details for a consumer's saved gift certificate.

Listed below is a sample [Get Stored Gift Certificate by ID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-gift-certificates-by-id-get) GET request URI. It is not JWT-restricted.
...
Listed below is a [Get Stored Payment by ID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-modify-credit-card-stored-payment-get) GET request URI. **This endpoint is JWT-restricted**.
...
Use the [Validate Stored Payment Credit Card CVV](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-modify-credit-card-stored-payment-post) endpoint to validate a credit card or gift certificate based on the request shipping address. If the shipping address does not match a shipping address on a past order or is not sent, the response indicates that the CVV needs to be validated. This endpoint also validates if a credit card is expired. Note that billing address is returned.

Listed below is a sample [Validate Stored Payment Credit Card CVV](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-modify-credit-card-stored-payment-post) POST request URI. **This endpoint is JWT-restricted**.
...
In order to save PayPal as a stored payment, the consumer must [Start a PayPal Billing Agreement](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-start-a-paypal-billing-agreement-get). This agreement pre-authorizes Nike to charge the consumer’s PayPal account for purchases without requiring the consumer to visit the PayPal site to authorize each purchase. Once the consumer sets up a PayPal billing agreement, purchasing by PayPal is easy because the consumer can stay in your experience without having to go the PayPal site.

Follow these steps to allow consumers to Add a PayPal stored payment in your experience.

1. Call [Get Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-payments-by-upmid-post) to get a list all the consumer's saved payments to make sure the consumer doesn't already have a PayPal stored payment. Consumers can have only one PayPal stored payment.

2. Call the [Start a PayPal Billing Agreement](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-start-a-paypal-billing-agreement-get) endpoint, passing the `returnURL` and `cancelURL` as query parameters so PayPal can return the consumer to your experience.
...
4. Call the [Add Stored Payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-add-stored-payment-post) endpoint to save the PayPal billing agreement.

5. (Optional) Call [Get Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-payments-by-upmid-post) to list all the consumer's saved payments.

Listed below is a sample [Start a PayPal Billing Agreement](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-start-a-paypal-billing-agreement-get) GET request URI for the SNKRS mobile experience. This endpoint requires a service-to-service JWT in the X-Nike-Authorization header.
...
In order to save a Fiserv credit card as a stored payment, the consumer must [Start a Fiserv Billing Key Registration](https://console.platforms.nike.com/developer/docs/projects/Stored%20Payments?tab=api#start-a-fiserv-billkey-registration-v1-post). This agreement pre-authorizes Nike to charge the consumer’s Fiserv credit card for purchases. Storing their Fiserv credit card is convenient for customers because they do not need to authenticate at the Fiserv site during the checkout flow. This way, consumers can stay in your experience and checkout faster and easier.
...
1. Call [Get Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-payments-by-upmid-post) to check if the stored payment exists. If so, skip the rest of this section and proceed to [Payment Preview](#payment-preview).

2. Make a POST call to the [Start a Fiserv Bill Key Registration](https://console.platforms.nike.com/developer/docs/projects/Stored%20Payments?tab=api#start-a-fiserv-billkey-registration-v1-post) endpoint, passing the billing and shipping information, `returnURL` and `cancelURL` so Fiserv can return the consumer to your experience in **Step 3**. A successful response contains the Fiserv `url` and the `fields` object consisting of name/value pairs.
...
4. Make a POST call to the [Save a Fiserve Bill Key Registration](https://console.platforms.nike.com/developer/docs/projects/Stored%20Payments?tab=api#start-a-fiserv-billkey-registration-v1-post-1) endpoint to save the Fiserve bill key registration and payment details.

5. (Optional) Call [Get Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api#stored-payment-get-stored-payments-by-upmid-post) again to list all of the consumer's saved payments to make sure the newly added Fiserve credit card is in the list.
...
The [Add Credit Card Info with CVV](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-add-credit-card-with-cvv-get) endpoint is called by experiences that are not PCI-certified. This endpoint renders an iFrame with editable masked credit card number, expiration date, and CVV fields pre-populated with values matching the `creditCardInfoId` passed in the path parameter. If the `creditCardInfoId` is not found, the iFrame renders blank, editable credit card number, expiration date and CVV fields. When each field has a value, the iFrame calls the [Store Credit Card for Validation and Purchase](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-store-credit-card-for-validation-and-purchase-post) endpoint and temporarily stores the new or updated values. As a last step, the iFrame calls the [Validate Credit Card](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-validate-credit-card-get) endpoint to validate the new or updated credit card data.

>**TIP:** When calling this endpoint through the public router, the `upmid` (for logged-in consumers), `appId` and `usertype` headers are automatically added by the Nike Edge Router based on the access token in the Authorization header populated by [accounts.nike.com](https://miniature-couscous-57c7acad.pages.github.io/) or [Nike Unite/Identity](https://confluence.nike.com/display/USER/Unite+Platform+-+Product+Documentation).

Listed below is a sample [Add Credit Card Info with CVV](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-add-credit-card-with-cvv-get) GET request URI. It is not JWT-restricted.
...
The [Add Credit Card Info without CVV](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-add-credit-card-without-cvv-get) endpoint is called by experiences that are not PCI-certified. This endpoint renders an iFrame with the editable masked credit card number and expiration date fields matching the `creditCardInfoId` passed in the path parameter. If the `creditCardInfoId` is not found, the iFrame renders blank, editable credit card number and date fields. When each field has a value, the iFrame calls the [Store Credit Card for Validation and Purchase](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-store-credit-card-for-validation-and-purchase-post) endpoint and temporarily stores the new or updated values. As a last step, the iFrame calls the [Validate Credit Card](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-validate-credit-card-get) endpoint to validate the new or updated credit card data.

>**TIP:** When calling this endpoint through the public router, the `upmid` (for logged-in consumers), `appId` and `usertype` headers are automatically added by the Nike Edge Router based on the access token in the Authorization header populated by [accounts.nike.com](https://miniature-couscous-57c7acad.pages.github.io/) or [Nike Unite/Identity](https://confluence.nike.com/display/USER/Unite+Platform+-+Product+Documentation).

Listed below is a sample [Add Credit Card Info without CVV](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-add-credit-card-without-cvv-get) GET request URI. It is not JWT-restricted.
...
The [Add or Update Credit Card CVV](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-add-or-update-credit-card-cvv-get) endpoint is called by experiences that are not PCI-certified. This endpoint renders an iFrame with an editable CVV field. When the consumer provides a CVV value, the iFrame calls the [Store Credit Card for Validation and Purchase](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-store-credit-card-for-validation-and-purchase-post) endpoint and temporarily stores the CVV if it found a credit card matching the `creditCardInfoId` path parameter. As a last step, the iFrame calls the [Validate Credit Card](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-validate-credit-card-get) endpoint to validate the updated CVV.

>**TIP:** When calling this endpoint through the public router, the `upmid` (for logged-in consumers), `appId` and `usertype` headers are automatically added by the Nike Edge Router based on the access token in the Authorization header populated by [accounts.nike.com](https://miniature-couscous-57c7acad.pages.github.io/) or [Nike Unite/Identity](https://confluence.nike.com/display/USER/Unite+Platform+-+Product+Documentation).

Listed below is a sample [Add or Update Credit Card CVV](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-add-or-update-credit-card-cvv-get) GET request URI. It is not JWT-restricted.
...
The [Add or Update Credit Card Expiry and CVV](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-add-or-update-credit-card-expiry-and-cvv-get) endpoint is called by experiences that are not PCI-certified. This endpoint renders an iFrame with an editable credit card expiration date and CVV fields. When the consumer provides the appropriate values, the iFrame calls the [Store Credit Card for Validation and Purchase](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-store-credit-card-for-validation-and-purchase-post) endpoint and temporarily stores the data for the `creditCardInfoId` path parameter. As a last step, the iFrame calls the [Validate Credit Card](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-validate-credit-card-get) endpoint to validate the updated expiration date and CVV values.

>**TIP:** When calling this endpoint through the public router, the `upmid` (for logged-in consumers), `appId` and `usertype` headers are automatically added by the Nike Edge Router based on the access token in the Authorization header populated by [accounts.nike.com](https://miniature-couscous-57c7acad.pages.github.io/) or [Nike Unite/Identity](https://confluence.nike.com/display/USER/Unite+Platform+-+Product+Documentation).

Listed below is a sample [Add or Update Credit Card Expiry and CVV](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-add-or-update-credit-card-expiry-and-cvv-get) GET request URI. It is not JWT-restricted.
...
The [Validate Credit Card](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-validate-credit-card-get) endpoint is typically called by the credit card payment iFrames immediately after the [Store Credit Card for Validation and Purchase](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-store-credit-card-for-validation-and-purchase-post) endpoint to validate credit card information. The mode query parameter determines which credit card values to validate. It can be called directly by PCI-cerfified experiences.
...
Listed below is a sample [Validate Credit Card](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-validate-credit-card-get) GET request URI. It is not JWT-restricted.
...
Sample [Validate Credit Card](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-validate-credit-card-get) response body for mode=1:
...
Sample [Validate Credit Card](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-validate-credit-card-get) response body for mode=2:
...
Sample [Validate Credit Card](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-validate-credit-card-get) response body for mode=3:
...
```
Sample [Validate Credit Card](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-validate-credit-card-get) response body for mode=4:
```
...
The [Store Credit Card for Validation and Purchase](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-store-credit-card-for-validation-and-purchase-post) endpoint temporarily stores credit card information for validation and purchase. This endpoint is called by the credit card payment iFrames. It can be called directly by PCI-certified experiences.

Listed below is a sample [Store Credit Card for Validation and Purchase](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-store-credit-card-for-validation-and-purchase-post) POST request URI. It is not JWT-restricted.
...
The [Get Credit Card](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-get-credit-card-get) endpoint retrieves masked credit card information for a `creditCardInfoId`. It can be called directly by PCI-certified experiences.


Listed below is a [Get Credit Card](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-get-credit-card-get) GET request URI. It is not JWT-restricted.
...
The [Get and Validate Credit Card](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-get-and-validate-credit-card-get) endpoint retrieves masked credit card information and validation status of individual credit card fields for a `creditCardInfoId`. It can be called directly by PCI-certified experiences.


Listed below is a [Get and Validate Credit Card](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-get-and-validate-credit-card-get) GET request URI. It is not JWT-restricted.
```
...
We recommend reading [Apple Pay on the Web](https://developer.apple.com/documentation/apple_pay_on_the_web) and [Apple Pay JS API](https://developer.apple.com/documentation/apple_pay_on_the_web/apple_pay_js_api) first.

#### Step 1: Check That the Consumer Can Pay By Apple Pay

To enable paying with Apple Pay on a Safari Web browser, your experience will need to call the [Apple Pay JS API](https://developer.apple.com/documentation/apple_pay_on_the_web/apple_pay_js_api) to validate that the Nike consumer can pay by Apple Pay on the web and get a `validationURL` to provide merchant identification to Apple.
...
Use the [Start Apple Pay Session](https://console.platforms.nike.com/developer/docs/projects/Payment%20ApplePay?tab=api#payment-applepay-start-apple-pay-payment-session-post) endpoint to initialize an Apple Pay session through the Apple gateway. Your experience passes the `validationURL` from **Step 1** to the endpoint, and the service will provide the necessary information to Apple Pay to identify Nike as a merchant that accepts Apple Pay payments and start a new Apple Pay session.

#### Step 3: Store Credit Card for Validation and Purchase

Once you get a successful 200 response from [Start an Apple Pay Session](https://console.platforms.nike.com/developer/docs/projects/Payment%20ApplePay?tab=api#payment-applepay-start-apple-pay-payment-session-post), you have all the information you need to call [Store Credit Card for Validation and Purchase](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-store-credit-card-for-validation-and-purchase-post) and continue the purchase flow as you would for a credit card.

>**TIP:** When calling this endpoint through the public router, the `upmid` (for logged-in consumers), `appId` and `usertype` headers are automatically added by the Nike Edge Router based on the access token in the Authorization header populated by [accounts.nike.com](https://miniature-couscous-57c7acad.pages.github.io/) or [Nike Unite/Identity](https://confluence.nike.com/display/USER/Unite+Platform+-+Product+Documentation).

Listed below is a sample [Start Apple Pay Session](https://console.platforms.nike.com/developer/docs/projects/Payment%20ApplePay?tab=api#payment-applepay-start-apple-pay-payment-session-post) POST request URI and body. The `validationURL` is passed to your experience from the Apple Pay JS API when you [provide merchant validation](https://developer.apple.com/documentation/apple_pay_on_the_web/apple_pay_js_api/providing_merchant_validation).
...
For v2 Checkout, use the [Request PayPal Express v1](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet?tab=api#paypal-express-service-request-paypal-express-post) endpoint:

`POST https://api.nike.com/payment/paypal_express/v1`

**v3 Checkout**

For v3 Checkout, use the [Request PayPal Express v2](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet%20V2%20(Source%20Aware)?tab=api#paypal-express-service-request-paypal-express-1) endpoint:
...
>**TIP**: The `returnURL` you provide in the [Request PayPal Express](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet?tab=api#paypal-express-service-request-paypal-express-post) **request body** is a link to your experience to which PayPal will redirect the consumer after confirming the payment method and shipping address. The `returnURL` provided in the [Request PayPal Express](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet?tab=api#paypal-express-service-request-paypal-express-post) **response body** is a link to the PayPal site to which your experience will redirect the consumer to select a shipping address and payment method.
...
For v2 Checkout, use the [Retrieve PayPal Express Job v1](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet?tab=api#paypal-express-service-retrieve-paypal-express-job-get) endpoint:

`GET https://api.nike.com/payment/paypal_express/v1/jobs/2722be3a-0341-11e6-b512-3e1d05defe783424`

**v3 Checkout**

For v3 Checkout, use the [Retrieve PayPal Express Job v2](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet%20V2%20(Source%20Aware)?tab=api#paypal-express-service-retrieve-paypal-express-job-1) endpoint:
...
For v2 Checkout, use the [Request PayPal Details v1](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet?tab=api#paypal-details-service-request-paypal-details-1) endpoint:

`POST https://api.nike.com/payment/paypal_details/v1`

**v3 Checkout**

For v3 Checkout, use the [Request PayPal Details v2](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet%20V2%20(Source%20Aware)?tab=api#paypal-details-service-request-paypal-details-1) endpoint:
...
For v2 Checkout, use the [Request PayPal Mark v1](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet?tab=api#paypal-mark-service-request-paypal-mark-post) endpoint:

`POST https://api.nike.com/payment/paypal_mark/v1`

**v3 Checkout**

For v3 Checkout, use the [Request PayPal Mark v2](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet%20V2%20(Source%20Aware)?tab=api#paypal-mark-service-request-paypal-mark-1) endpoint:
...
>**TIP**: The `returnURL` you provide in the [Request PayPal Mark](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet?tab=api#paypal-mark-service-request-paypal-mark-post) **request body** is a link to your experience to which PayPal will redirect after the consumer pays for their Checkout at the PayPal site. The `returnURL` provided in the [Request PayPal Mark](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet?tab=api#paypal-mark-service-request-paypal-mark-post) **response body** is a link to PayPal to which your experience will redirect the consumer to pay.
...
For v2 Checkout, use the [Retrieve PayPal Mark Job v1](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet?tab=api#paypal-mark-service-retrieve-paypal-mark-job-1) endpoint:

`GET https://api.nike.com/payment/paypal_mark/v1/jobs/2722be3a-0341-11e6-b512-3e1d05defe783424`

**v3 Checkout**

For v3 Checkout, use the [Retrieve PayPal Mark Job v2](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet%20V2%20(Source%20Aware)?tab=api#paypal-mark-service-retrieve-paypal-mark-job-1) endpoint:
...
Use the [Request Deferred Payment Form](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api#deferred-payment-form-request-deferred-payment-form-post) endpoint to generate a signed link used to redirect the consumer to pay at a third-party site or app. This endpoint is used for experiences that support these payment types:
- iDeal
...
In the request body, your experience will need to pass the `approvalId` returned from [Request Payment Approval](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-request-payment-approval-post) and your experience's `returnURL` that the third-party vendor will redirect the consumer to after making payment at their site.

This endpoint operates **asynchronously** which means that there are extra steps to retrieve the results of your request. Read the Asynchronous Operation section of the [Using Nike APIs](/doc/getting-started/using-nike-apis.html#asynchronous-operation) guide to learn more about working with asynchronous Nike APIs.

Listed below is a sample [Request Deferred Payment Form](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api#deferred-payment-form-request-deferred-payment-form-post) POST request URI. The endpoint is not JWT-restricted.
...
Use the [Retrieve Deferred Payment Form Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api#deferred-payment-form-retrieve-deferred-payment-form-job-get) endpoint to check the status of the [Request Deferred Payment Form](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api#deferred-payment-form-request-deferred-payment-form-post) job. After receiving a HTTP 202 and waiting the duration of the ETA time, call this endpoint using the same UUID to check the status of your job. If the status is not COMPLETED, continue the cycle of waiting the ETA period and checking the job status. In the case of checking the status of [Request WeChat Deferred Payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api#wechat-deferred-payment-request-wechat-deferred-payment-post), see the [Request WeChat Deferred Payment](#step-2-request-wechat-deferred-payment) section.
...
Listed below is a sample [Retrieve Deferred Payment Form Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api#deferred-payment-form-retrieve-deferred-payment-form-job-get) GET request URI. The endpoint is not JWT-restricted.
...
Use the values from the [Request Deferred Payment Form](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api#deferred-payment-form-request-deferred-payment-form-post) job to redirect the consumer to pay at the third-party site. Depending upon the vendor and the experience the consumer is shopping in, the response may contain a URL to generate a QR code for the deferred payment page, or a form action URL and HTTP method.

#### Step 4: Request Deferred Payment Status

Use the [Request Deferred Payment Status](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api#deferred-payment-status-request-deferred-payment-status-post) endpoint to check the status of a deferred payment with the third-party Vendor.

Your experience will need to pass the `approvalId` returned from [Request Payment Approval](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-request-payment-approval-post) and any `vendorData` returned in the [Request Deferred Payment Form](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api#deferred-payment-form-request-deferred-payment-form-post) response.

This endpoint operates **asynchronously** which means that there are extra steps to retrieve the results of your request. Read the Asynchronous Operation section of the [Using Nike APIs](/doc/getting-started/using-nike-apis.html#asynchronous-operation) guide to learn more about working with asynchronous Nike APIs.

Listed below is a sample [Request Deferred Payment Status](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api#deferred-payment-status-request-deferred-payment-status-post) POST request URI. This endpoint requires a service-to-service JWT in the X-Nike-Authorization header.
...
Use the [Retrieve Deferred Payment Status Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api#deferred-payment-status-retrieve-deferred-payment-status-job-get) endpoint to check the status of the [Request Deferred Payment Status](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api#deferred-payment-status-request-deferred-payment-status-post). After receiving a HTTP 202 and waiting the duration of the ETA time, call this endpoint using the same UUID to check the status of your job. If the status is not COMPLETED, continue the cycle of waiting the ETA period and checking the job status.
...
Listed below is a sample [Retrieve Deferred Payment Status Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api#deferred-payment-status-retrieve-deferred-payment-status-job-get) GET request URI. This endpoint requires a service-to-service JWT in the X-Nike-Authorization header.
...
We recommend reading [JSAPI WeChat Browser](https://confluence.nike.com/pages/viewpage.action?spaceKey=ocp&title=jsapi+wechat+browser) first.
...
Using the code returned from WeChat in **Step 1** and the `approvalId` returned in the [Request Payment Approval](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-request-payment-approval-post) response, call [Request WeChat Deferred Payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api#wechat-deferred-payment-request-wechat-deferred-payment-post-1).

Listed below is a sample [Request WeChat Deferred Payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api#wechat-deferred-payment-request-wechat-deferred-payment-post-1) POST request URI. This endpoint is not JWT-restricted.
```
...
Use the [Retrieve WeChat Deferred Payment Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api#wechat-deferred-payment-retrieve-wechat-deferred-payment-job-get) endpoint to check the status of the [Request WeChat Deferred Payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api#wechat-deferred-payment-request-wechat-deferred-payment-post) job. After receiving a HTTP 202 and waiting the duration of the ETA time, call this endpoint using the same UUID to check the status of your job. If the status is not COMPLETED, continue the cycle of waiting the ETA period and checking the job status.
...
Listed below is a sample [Retrieve Deferred WeChat Payment Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api#wechat-deferred-payment-retrieve-wechat-deferred-payment-job-get) GET request URI. It is not JWT-restricted.
...
Listed below is a sample [Request Ready Payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Korea?tab=api#ready-payment-put) POST request URI. This endpoint is asynchronous.
...
Use the [Retrieve Ready Payment Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Korea?tab=api#ready-payment-get) endpoint to check the status of the [Request Ready Payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Korea?tab=api#ready-payment-put). After receiving a HTTP 202 and waiting the duration of the ETA time, call this endpoint using the same UUID job `id` to check the status of your job. If the status is not COMPLETED, continue the cycle of waiting the ETA period and checking the job status.
...
Listed below is a sample [Retrieve Ready Payment Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Korea?tab=api#ready-payment-get) GET request URI with 621827cc-82b4-408b-9e63-7292795fa233 as the `id` path parameter.
...
>**TIP**: Scroll to the bottom of the [KakaoPay Integration Confluence page](https://confluence.nike.com/display/PHYLON/KakaoPay+integration) to view the KakaoPay Developer guide for more information.
...
>**TIP**: Scroll to the bottom of the [Naver Pay Integration Confluence page](https://confluence.nike.com/display/PHYLON/Naver+Pay+Integration) to view the Naver Pay Integration guide for more information.
...
>**TIP**: Scroll to the bottom of the [Fiserv Credit Card Integration Confluence page](https://confluence.nike.com/display/PHYLON/Credit+card+integration) to view the Fiserv UI Integration guide for more information.
...
For v2 Checkout, use the [Payment Preview v2](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview?tab=api#payment-preview-request-payment-preview) endpoint:
...
For v3 Checkout, use the [Request Payment Preview v3](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview%20V3%20(Source%20aware)?tab=api#payment-preview-request-payment-preview) endpoint:
...
- The `paymentPreviewId` returned by this service is a required key when calling [Request Checkout Submit](https://console.platforms.nike.com/developer/docs/projects/Checkouts%20V2?tab=api#checkout-request-a-checkout-submit-put) to validate and authorize/debit payment before submitting a Checkout to Nike for fulfillment.
- This endpoint operates **asynchronously** which means that there are extra steps to retrieve the results of your request. Read the Asynchronous Operation section of the [Using Nike APIs](/doc/getting-started/using-nike-apis.html#asynchronous-operation) guide to learn more about working with asynchronous Nike APIs.
- When calling this endpoint through the public router, the `upmid` (for logged-in consumers), `appId` and `usertype` headers are automatically added by the Nike Edge Router based on the access token in the Authorization header populated by [accounts.nike.com](https://miniature-couscous-57c7acad.pages.github.io/) or [Nike Unite/Identity](https://confluence.nike.com/display/USER/Unite+Platform+-+Product+Documentation).
...
- `checkoutId` is a [UUID](https://en.wikipedia.org/wiki/Universally_unique_identifier) that payments are associated to. It is generated by [Request Checkout Preview](https://console.platforms.nike.com/developer/docs/projects/Checkouts%20V2?tab=api#checkout-preview-request-checkout-preview-put).
...
- `paymentInfo` is an array of payment types for the Checkout and is required except for the [PayPal Mark](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet?tab=api#paypal-mark-service-request-paypal-mark-post) flow.
- paymentInfo.`billingInfo` is required for all payment methods except [PayPal](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet?tab=api).
- PaymentInfo.`creditCardInfoId` is a required field when paying by credit card that is not a [stored payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api). It is a PCI-required token used to look up credit card information and is generated by the [Store Credit Card for Validation and Purchase](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api#credit-card-information-store-credit-card-for-validation-and-purchase-post) endpoint. If the credit card is a stored payment, then PaymentInfo.**paymentId** is required.
...
For v2 Checkout, use the [Retrieve Paymen Preview Job v2](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview?tab=api#payment-preview-retrieve-payment-preview-job) endpoint:

`GET https://api.nike.com/payment/preview/v2/jobs/2722be3a-0341-11e6-b512-3e1d05defe783424`

**v3 Checkout**

For v3 Checkout, use the [Retrieve Payment Preview Job v3](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview%20V3%20(Source%20aware)?tab=api#payment-preview-retrieve-payment-preview-job) endpoint:
...
Once you receive a job status of "COMPLETED", get the results of your job by parsing the data in the response object from this endpoint. Alternatively, follow the link to the [Retrieve Payment Preview Result](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview?tab=api#payment-preview-retrieve-payment-preview-result-get) endpoint which is provided in the `links` object response body.
...
After calling the [Request Payment Preview](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview?tab=api#payment-preview-request-payment-preview-post) to start the job and [Retrieve Payment Preview Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview?tab=api#payment-preview-retrieve-payment-preview-job-get) to check that the status of the job is "COMPLETED", you can optionally call the [Retrieve Payment Preview Result](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview?tab=api#payment-preview-retrieve-payment-preview-result-get) endpoint to retrieve the job result. It is optional because the result is also returned in the [Retrieve Payment Preview Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview?tab=api#payment-preview-retrieve-payment-preview-job-get) when it is in "COMPLETED" status.

Listed below is a sample [Retrieve Payment Preview Result](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview?tab=api#payment-preview-retrieve-payment-preview-result-get) GET request URI. It is not JWT-restricted.
...
The Payment 3DS service (3-Domain Secure) adds a layer of protection against fraud in credit card and debit card transactions. It uses [Adyen](https://www.adyen.com/risk-management/3d-secure-2-0), a third-party 3D Secure 2 provider, to authenticate payment transactions. Not all credit card payment transactions require 3DS. If 3DS is required, your experience calls the Payment 3DS service as a separate step before payment authorization, which takes place during [Request Checkout Submit](https://console.platforms.nike.com/developer/docs/projects/Checkouts%20V2?tab=api#checkout-request-a-checkout-submit-put).

**When do I need to call this service?**

If `is3DSRequired` is **true** in the [Payment Preview](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview?tab=api#payment-preview-retrieve-payment-preview-job-get) response, the transaction requires 3DS authentication. In this case, your experience needs to call the [Request Authentication](https://console.platforms.nike.com/developer/docs/projects/Payment3DS?tab=api#request-authentication-post) endpoint in the Payment 3DS API. Depending upon the response, you will either call additional 3DS and Adyen endpoints or proceed directly to [Request Checkout Submit](https://console.platforms.nike.com/developer/docs/projects/Checkouts%20V2?tab=api#checkout-request-a-checkout-submit-put).

>**TIP:** All 3DS POST endpoints are synchronous.

#### Step 1: Request Authentication

Call the [Request Authentication](https://console.platforms.nike.com/developer/docs/projects/Payment3DS?tab=api#request-authentication-post) endpoint passing the `paymentPreviewId` from the [Request Payment Preview](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview?tab=api#payment-preview-request-payment-preview-post) response, currency, originURL, returnURL, channel, amount, and browser information.

Listed below is a sample [Request Authentication](https://console.platforms.nike.com/developer/docs/projects/Payment3DS?tab=api#request-authentication-post) POST request URI. **This endpoint is JWT-restricted.**
...
If the [Request Authentication](https://console.platforms.nike.com/developer/docs/projects/Payment3DS?tab=api#request-authentication-post) call returned a resultCode of **IdentifyShopper**, you will need to get the secure device fingerprint.

##### Step 2a: Get the Fingerprint Result Token

Follow [Adyen's fingerprint flow for Web, iOs or Android](https://docs.adyen.com/online-payments/3d-secure/native-3ds2/api-integration#get-the-3d-secure-2-device-fingerprint) in your app or experience passing the `token` as the fingerprintToken from the [Request Authentication](https://console.platforms.nike.com/developer/docs/projects/Payment3DS?tab=api#request-authentication-post) response in **Step 1**. A successful response returns the device fingerprint result token.

##### Step 2b: Get the Fingerprint

Once you have the device fingerprint result token from **Step 2a**, call the [Request Fingerprint](https://console.platforms.nike.com/developer/docs/projects/Payment3DS?tab=api#request-fingerprint-post) endpoint passing the `paymentPreviewId` from the [Request Payment Preview](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview?tab=api#payment-preview-request-payment-preview-post) response, and the device fingerprint result token.

Listed below is a sample [Request Fingerprint](https://console.platforms.nike.com/developer/docs/projects/Payment3DS?tab=api#request-fingerprint-post) POST request URI. **This endpoint is JWT-restricted.**
...
If the [Request Fingerprint](https://console.platforms.nike.com/developer/docs/projects/Payment3DS?tab=api#request-fingerprint-post) call returned a resultCode of **ChallengeShopper**, you will need to present an authentication challenge to the consumer.

##### Step 3a: Present a Challenge

Follow [Adyen's present a challenge flow for Web, iOs or Android](https://docs.adyen.com/online-payments/3d-secure/native-3ds2/api-integration#present-a-challenge) in your app or experience passing the `token` from the [Request Fingerprint](https://console.platforms.nike.com/developer/docs/projects/Payment3DS?tab=api#request-fingerprint-post) response from **Step 2** as the challenge token. A successful response returns a challenge result token.

##### Step 3b: Request Challenge

Once you have the challenge result token from **Step 3a**, call the [Request Challenge](https://console.platforms.nike.com/developer/docs/projects/Payment3DS?tab=api#request-fingerprint-post) endpoint passing the `paymentPreviewId` from the [Request Payment Preview](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview?tab=api#payment-preview-request-payment-preview-post) response, and the challenge result token as the `challengeResultToken`.

Listed below is a sample [Request Challenge](https://console.platforms.nike.com/developer/docs/projects/Payment3DS?tab=api#request-fingerprint-post) POST request URI. **This endpoint is JWT-restricted.**
...
If the [Request Authentication](https://console.platforms.nike.com/developer/docs/projects/Payment3DS?tab=api#request-authentication-post) call returned a resultCode of **RedirectShopper**, you will need to redirect the consumer to the issuer’s site to authenticate the transaction using 3DS 1 as a fallback.

##### Step 4a: Redirect the Consumer to the Issuer's Site

Redirect the consumer to the `url` from the [Request Authentication](https://console.platforms.nike.com/developer/docs/projects/Payment3DS?tab=api#request-authentication-post) response, so the consumer can complete payment authentication. Once the payment is successfully authenticated at the bank site, the consumer will be redirected to your site with `MD` and `PaRes` variables appended.

##### Step 4b: Request Redirect

After the transaction was successfully authenticated at the issuer's site in **Step 4a**, call the [Request Redirect](https://console.platforms.nike.com/developer/docs/projects/Payment3DS?tab=api#request-redirect-post) endpoint passing  the `paymentPreviewId` from the [Request Payment Preview](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview?tab=api#payment-preview-request-payment-preview-post) response and `MD` and `PaRes` URL parameters returned in **Step 4a**.

Listed below is a sample [Request Redirect](https://console.platforms.nike.com/developer/docs/projects/Payment3DS?tab=api#request-redirect-post) POST request URI. **This endpoint is JWT-restricted.**
...
Once the 3DS transaction has been authenticated, follow the steps for [Request Checkout Submit](/doc/commerce/checkout/use-checkout.html#checkout-submit) when the consumer is ready to complete the purchase.
...
>**TIP:** Except for the [Get Payment Approval Summary](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-get-payment-approval-summary-get) endpoint, only service-to-service calls should be made to the Payment Approval endpoints. [Request Checkout Submit](https://console.platforms.nike.com/developer/docs/projects/Checkouts%20V2?tab=api#checkout-request-a-checkout-submit-put) calls [Request Payment Approval (POST)](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-request-payment-approval-post) as a last step in the Checkout flow to validate and authorize/debit payment before submitting a Checkout to Nike for fulfillment. **An experience should not call this service directly**.
...
For v2 Checkout, use the [Request Payment Approval v2](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-request-payment-approval) endpoint:
...
For v3 Checkout, use the [Request Payment Approval v3](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval%20V3%20(Source%20Aware)?tab=api#payment-approval-request-payment-approval) endpoint:
...
- The PUT version of the endpoint requires a `paymentApprovalId` path parameter. This is helpful if the [Request Payment Approval](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-request-payment-approval-put) response times out. In that case, [Request Payment Void](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-request-payment-void-delete) would need to be called with the same `paymentApprovalId` to reverse the original Payment Approval request.
- This endpoint (POST or PUT) must be called after [Request Checkout Preview](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview?tab=api#payment-preview-request-payment-preview-post) so that order allocation is calculated and the `paymentPreviewId` is assigned.
- The Payment Approval service validates the payment allocation performed by the [Payment Preview](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview?tab=api#payment-preview-request-payment-preview-post) service, recalculating if necessary, and evaluates that the selected payment methods and items on Checkout are valid. If one or more payment type validations fail, all gift card debits and all credit card and PayPal authorizations are rolled back. This service uses the `paymentPreviewId` to look up the Checkout payment methods, so it does not require payment information be passed in the request.
- This endpoint operates **asynchronously**, which means that there are extra steps to retrieve the results of your request. Read the Asynchronous Operation section of the [Using Nike APIs](/doc/getting-started/using-nike-apis.html#asynchronous-operation) guide to learn more about working with asynchronous Nike APIs.
- A successful 202 response includes a link to the job and a status polling ETA.
...
Use this endpoint to check the status of the [Request Payment Approval](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-request-payment-approval-post) (POST) or [Request Payment Approval](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-request-payment-approval-put) (PUT) job. After receiving a HTTP 202 and waiting the duration of the ETA time specified in the response, call [Retrieve Payment Approval Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-retrieve-payment-approval-job-get) using the same UUID to check the status of your job. If the status is not COMPLETED, continue the cycle of waiting the ETA period and checking the job status.

**v2 Checkout**

For v2 Checkout, use the [Retrieve Payment Approval Job v2](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-retrieve-payment-approval-job) endpoint:

`GET https://api.nike.com/payment/approval/v2/jobs/2722be3a-0341-11e6-b512-3e1d05defe783424`

**v3 Checkout**

For v3 Checkout, use the [Retrieve Payment Approval Job v3](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval%20V3%20(Source%20Aware)?tab=api#payment-approval-retrieve-payment-approval-job) endpoint:
...
After calling [Request Payment Approval](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-request-payment-approval-post) (POST) or [Request Payment Approval](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-request-payment-approval-put) (PUT) to start the job and [Retrieve Payment Approval Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-retrieve-payment-approval-job-get) to verify the status of the job is "COMPLETED", you can optionally call [Retrieve Payment Approval Results](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-retrieve-payment-approval-results-get) to retrieve the result of your Payment Approval job. This step is optional because the Payment Approval result is also returned in the [Retrieve Payment Approval Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-retrieve-payment-approval-job-get) when it is in `COMPLETED` status. DOMS is currently the only service that calls this endpoint.

**v2 Checkout**

For v2 Checkout, use the [Retrieve Payment Approval Result v2](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-retrieve-payment-approval-results) endpoint:

GET `https://api.nike.com/payment/approval_results/v2/ae6575a7-8c0e-44ef-b91b-440bdaf2070b`

**v3 Checkout**

For v3 Checkout, use the [Retrieve Payment Approval Result v2](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval%20V3%20(Source%20Aware)?tab=api#payment-approval-retrieve-payment-approval-results) endpoint:
...
Use the [Request Payment Void](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-request-payment-void-delete) endpoint to void a [Request Payment Approval](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-request-payment-approval-post). This endpoint should be called when the original [Request Payment Approval](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-request-payment-approval-put) (PUT) timed out using the same {id} sent in the path parameter. If a credit card or PayPal was used in the original Payment Approval request, this endpoint reverses the authorization. If a gift card was used in the original Payment Approval request, it reverses the debit.

**v2 Checkout**

For v2 Checkout, use the [Request Payment Void v2](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-request-payment-void) endpoint:

DELETE `https://api.nike.com/payment/approval_results/v2/ae6575a7-8c0e-44ef-b91b-440bdaf2070b`

**v3 Checkout**

For v3 Checkout, use the [Request Payment Void v3](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval%20V3%20(Source%20Aware)?tab=api#payment-approval-request-payment-void) endpoint:
...
Use the [Get Payment Approval Summary](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-get-payment-approval-summary-get) endpoint to retrieve a summary of a successful Payment Approval request. The results are available for 30 minutes after the original Payment Approval request was made. This endpoint can be called by an experience because sensitive account information is masked. The results can be used to display payment approval results confirming a consumer's order.
...
Listed below is a sample [Get Payment Approval Summary](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-get-payment-approval-summary-get) GET request URI. It is not JWT-restricted.
...
Use the [Request Debit](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#request-debit-put) endpoint to debit funds for orders paid by credit card, PayPal, Gift Certificate, and Klarna payment types. This endpoint is called when it is time to transfer funds from the consumer's account to Nike's account, such as when the consumer's shipment leaves the warehouse.

Listed below is a sample [Request Debit](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#request-debit-put) PUT request URI. **This endpoint requires a service-to-service JWT in the X-Nike-Authorization header**.

```
/payment/debits/v1/ae6575a7-8c0e-44ef-b91b-440bdaf2070b
```

>**TIP**: The `authorizationRequestId` and `authorizationRequestToken` request body fields come from the `requestId` and `requestToken` returned in the [Request Payment Approval](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-request-payment-approval-put) response.

A successful 202 response includes a link to the job and a status polling ETA.

#### **Step 2: Check the Debit Job Status**

Use the [Retrieve Debit Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-debit-job-get) endpoint to retrieve the results of the [Request Debit](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#request-debit-put) job. After receiving a HTTP 202 and waiting the duration of the ETA time specified in the response, call this endpoint using the same UUID to check the status of your job. If the status is not COMPLETED, continue the cycle of waiting the ETA period and checking the job status.
...
Listed below is a sample [Retrieve Debit Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-debit-job-get) GET request URI. This endpoint is not JWT-restricted.
```
...
When a Nike consumer returns one or more products, call the [Request Credit](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#submit-payment-request-for-credit-put) endpoint to return the money to the same account the consumer used to pay for the product. You can issue a full credit (refund) for the full charge amount, or you can issue multiple, partial credits up to the full charge amount. If you try to credit more than the charge amount, you will receive an error.

>**TIP**: You can get the `debitRequestId` and `debitRequestToken` values to pass in the credit request body from the `requestId` and `requestToken` fields in the [Retrieve Debit Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieval-payment-gateway-debit-job-get).

Listed below is a sample [Request Credit](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#submit-payment-request-for-credit-put) PUT request URI. **This endpoint requires a service-to-service JWT in the X-Nike-Authorization header**.
```
...
After calling [Request Credit](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#submit-payment-request-for-credit-put) and receiving a HTTP 202 response, execute a request to [Retrieve Credit Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-credit-job-get) using the same Credit ID to check the status of your job.
...
Listed below is a sample [Retrieve Credit Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-credit-job-get) GET request URI. This endpoint is not JWT-restricted.
```
...
Use the [Request Unauth](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#request-unauth-put) endpoint to release the hold on funds set aside by authorization for a future debit. See the [Payment Gateway API](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#request-unauth) for details on void request and response information for each payment type.

>**TIP**: The `authorizationRequestId` and `authorizationRequestToken` request body fields come from the `requestId` and `requestToken` in the [Request Payment Approval](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-request-payment-approval-put) response.

Listed below is a sample [Request Unauth](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#request-unauth-put) PUT request URI. **This endpoint requires a service-to-service JWT in the X-Nike-Authorization header**.
```
...
Use the [Retrieve Unauth Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-unauth-job-get) endpoint to retrieve the status of the [Request Unauth](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#request-unauth-put) job. After receiving a HTTP 202 and waiting the duration of the ETA time specified in the response, call this endpoint using the same UUID to check the status of your job. If the status is not `COMPLETED`, continue the cycle of waiting the ETA period and checking the job status.
...
Listed below is a sample [Retrieve Unauth Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-unauth-job-get) GET request URI. This endpoint is not JWT-restricted.
...
You can create an electronic gift certificate for a consumer by calling the [Create Gift Certificate](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#create-gift-certificate-put) endpoint. Pass the sender and recipient information, currency and amount in the request body.

Listed below is a sample [Create Gift Certificate](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#create-gift-certificate-put) PUT request URI. **This endpoint requires a service-to-service JWT in the X-Nike-Authorization header**.
```
...
After calling [Create Gift Certificate](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#create-gift-certificate-put) and receiving a HTTP 202 response, execute a request to [Retrieve Gift Certificate Create Job Status](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-gift-certificate-create-job-status-get) using the same {id} to check the status of your job. When the job has completed successfully, this endpoint returns an expiration date, gift certificate number, and PIN set to the amount and currency passed in the job request.
...
Listed below is a sample [Retrieve Gift Certificate Create Job Status](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-gift-certificate-create-job-status-get) GET request URI. It is not JWT-restricted.
...
You can create an electronic voucher for a consumer by calling the [Create Voucher](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#create-voucher-put) endpoint. Pass the sender and recipient information, currency and amount in the request body.

Listed below is a sample [Create Voucher](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#create-voucher-put) PUT request URI. **This endpoint requires a service-to-service JWT in the X-Nike-Authorization header**.
```
...
After calling [Create Voucher](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#create-voucher-put) and receiving a HTTP 202 response, execute a request to [Retrieve voucher create job status](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-voucher-create-job-status-get) using the same {id} to check the status of your job.
...
Listed below is a sample [Retrieve voucher create job status](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-voucher-create-job-status-get) GET request URI. It is not JWT-restricted.
...
Use the [Request Payment Status Query](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#request-payment-status-query-put) to check the payment status of an order. Before cancelling an order paid by a deferred payment type such as WeChat or Alipay, you will need to know if the order has been PAID so you can [Request Credit](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#request-credit-put) to return the funds to the consumer's account.

>**TIP**: The `requestId` and `requestToken` request body fields come from the `requestId` and `requestToken` in the [Request Payment Approval](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api#payment-approval-request-payment-approval-put) response.

Listed below is a sample [Request Payment Status Query](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#request-payment-status-query-put) PUT request URI. **This endpoint requires a service-to-service JWT in the X-Nike-Authorization header**.
...
After calling [Request Payment Status Query](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#request-payment-status-query-put) and receiving a HTTP 202 response, execute a request to [Retrieve Payment Status Query](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-payment-status-query-get) using the same {id} to check the status of your job.
...
Listed below is a sample [Retrieve Payment Status Query](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-payment-status-query-get) GET request URI. This endpoint requires a service-to-service JWT in the X-Nike-Authorization header.
...
Kick off a CyberSource report for your organization using the [Retrieve CyberSource Report](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-cybersource-report-post) endpoint. Provide your organization ID (i.e. merchant account), report name, and the requested report date. You do not need to send a report ID in the request. The service generates the ID for you and  returns it in the response.

Listed below is a sample [Retrieve CyberSource Report](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-cybersource-report-post) POST URI request. **This endpoint requires a service-to-service JWT in the X-Nike-Authorization header**.
...
After calling [Retrieve CyberSource Report](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-cybersource-report-post) and receiving a HTTP 202 response, execute a request to [Retrieve CyberSource Report Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-cybersource-report-job-get) using the job link in the response from **Step 1** to check the status of your job.
...
Listed below is a sample [Retrieve CyberSource Report Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-cybersource-report-job-get) GET request URI. It is not JWT-restricted.
...
Third-party payment vendors call the [Payment Notification API](https://github.com/nike-internal/payment.service.paymentnotification/blob/master/API.md) to notify Nike of payment status changes for an order with an offline payment type.
...
Each vendor has its own synchronous endpoint with the POST body defined by the vendor. The service response is either `success`, or `failure` and reason. See the [Payment Notification API](https://github.com/nike-internal/payment.service.paymentnotification/blob/master/API.md) for details on request and response by vendor.

## API Quick Reference

**Payment ApplePay**

- [Start Apple Pay Payment Session](https://console.platforms.nike.com/developer/docs/projects/Payment%20ApplePay?tab=api)

**Payment Approval**

v2:

- [Request Payment Approval](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api) (POST)
- [Request Payment Approval](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api) (PUT)
- [Retrieve Payment Approval Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api)
- [Retrieve Payment Approval Results](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api)
- [Request Payment Void](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api)
- [Get Payment Approval Summary](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval?tab=api)

v3:

- [Request Payment Approval](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval%20V3%20(Source%20Aware)?tab=api) (POST)
- [Request Payment Approval](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval%20V3%20(Source%20Aware)?tab=api) (PUT)
- [Retrieve Payment Approval Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval%20V3%20(Source%20Aware)?tab=api)
- [Retrieve Payment Approval Results](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval%20V3%20(Source%20Aware)?tab=api)
- [Request Payment Void](https://console.platforms.nike.com/developer/docs/projects/Payment%20Approval%20V3%20(Source%20Aware)?tab=api)

**Credit Card Payment**

- [Add Credit Card with CVV](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api)
- [Add Credit Card without CVV](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api)
- [Add or Update Credit Card CVV](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api)
- [Add or Update Credit Card Expiry and CVV](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api)
- [Validate Credit Card](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api)
- [Store Credit Card for Validation and Purchase](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api)
- [Get Credit Card](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api) (with validation)
- [Get and Validate Credit Card](https://console.platforms.nike.com/developer/docs/projects/Payment%20Credit%20Card%20Submit?tab=api) (without validation)

**Deferred Payment**

- [Request Deferred Payment Form](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api)
- [Retrieve Deferred Payment Form Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api)
- [Request Deferred Payment Status](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api)
- [Retrieve Deferred Payment Status Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api)
- [Request WeChat Deferred Payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api)
- [Retrieve WeChat Deferred Payment Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Deferred%20Payment?tab=api)

**Payment Gateway**

- [Request Debit](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#request-debit-put)
- [Retrieve Debit Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-debit-job-get)
- [Request Credit](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#request-credit-put)
- [Retrieve Credit Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-credit-job-get)
- [Request Unauth](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#request-unauth-put)
- [Retrieve Unauth Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-unauth-job-get)
- [Request Payment Status Query](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#request-payment-status-query-put)
- [Retrieve Payment Status Query](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-payment-status-query-get)
- [Create Gift Certificate](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#create-gift-certificate-put)
- [Retrieve Create Gift Certificate Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-gift-certificate-create-job-status-get)
- [Create Voucher](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#create-voucher-put)
- [Retrieve Voucher Create Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-voucher-create-job-status-get)
- [Retrieve CyberSource Report](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-cybersource-report-post)
- [Retrieve CyberSource Report Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Gateway?tab=api#retrieve-cybersource-report-job-get)

**Payment Options**

v2:

- [Get Payment Options](https://console.platforms.nike.com/developer/docs/projects/Payment%20Options?tab=api)
- [Get Billing Countries for Shipping Country](https://console.platforms.nike.com/developer/docs/projects/Payment%20Options?tab=api)
- [Validate Payments](https://console.platforms.nike.com/developer/docs/projects/Payment%20Options?tab=api)

v3:

- [Get Payment Options](https://console.platforms.nike.com/developer/docs/projects/Payment%20Options%20v3%20(Source%20Aware)?tab=api)

**Payment Preview**

v2:

- [Request Payment Preview](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview?tab=api)
- [Retrieve Payment Preview Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview?tab=api)
- [Retrieve Payment Results](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview?tab=api)

v3:

- [Request Payment Preview (POST)](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview%20V3%20(Source%20aware)?tab=api)
- [Request Payment Preview (PUT)](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview%20V3%20(Source%20aware)?tab=api)
- [Retrieve Payment Preview Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview%20V3%20(Source%20aware)?tab=api)
- [Retrieve Payment Results](https://console.platforms.nike.com/developer/docs/projects/Payment%20Preview%20V3%20(Source%20aware)?tab=api)

**Stored Payment**

- [Get Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api)
- [Get Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api) (Retail)
- [Get Stored Gift Certificates by ID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api)
- [Add Stored Payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api)
- [Delete Stored Payments by UPMID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api)
- [Modify Default Stored Payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api)
- [Modify Credit Card Stored Payment](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api)
- [Get Stored Payment by ID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api)
- [Delete Stored Payment by ID](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api)
- [Validate Stored Payment Credit Card CVV](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api)
- [Start a PayPal Billing Agreement](https://console.platforms.nike.com/developer/docs/projects/Payment%20Stored%20Payments?tab=api)
- [Start a FiServ Billkey Registration V1](https://console.platforms.nike.com/developer/docs/projects/Stored%20Payments?tab=api#start-a-fiserv-billkey-registration)
- [Save a FiServ Billkey Registration  V1](https://console.platforms.nike.com/developer/docs/projects/Stored%20Payments?tab=api#start-a-fiserv-billkey-registration-v1-post-1)

**Payment Wallet**

v2:

- [Request PayPal Details](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet?tab=api)
- [Retrieve PayPal Details Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet?tab=api)
- [Request PayPal Express](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet?tab=api)
- [Retrieve PayPal Express Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet?tab=api)
- [Request PayPal Mark](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet?tab=api)
- [Retrieve PayPal Mark Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet?tab=api)

v3:

- [Request PayPal Details](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet%20V2%20(Source%20Aware)?tab=api)
- [Retrieve PayPal Details Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet%20V2%20(Source%20Aware)?tab=api)
- [Request PayPal Express](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet%20V2%20(Source%20Aware)?tab=api)
- [Retrieve PayPal Express Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet%20V2%20(Source%20Aware)?tab=api)
- [Request PayPal Mark](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet%20V2%20(Source%20Aware)?tab=api)
- [Retrieve PayPal Mark Job](https://console.platforms.nike.com/developer/docs/projects/Payment%20Wallet%20V2%20(Source%20Aware)?tab=api)

**Korea Payment**

- [Ready Payment](https://console.platforms.nike.com/developer/docs/projects/payment_korea_read_payment_v1_api?tab=api#ready-payment-post)
- [Retrieve Ready Payment Job](https://console.platforms.nike.com/developer/docs/projects/payment_korea_read_payment_v1_api?tab=api#ready-payment-get)
...
In order to abide by PCI-compliance rules, payment logging requires special Splunk access. As a result, you cannot query Splunk by Trace ID as a trouble-shooting tool to track down why a payment request failed. If need help from the Payment Team, post your problem to the [#cic-payment](https://nikedigital.slack.com/messages/C0Z9P2E5Q) Slack channel with details such as:
...
Try using your browser's built-in tools for inspecting web service calls made from a live Nike experience such as [SNKRS Web](https://www.nike.com/launch). Or, set up Charles and your favorite device to proxy service calls made from SNKRS or other Nike apps. Sometimes seeing what other experiences are doing might address your question or concern.
...
|---|---|
|Slack|[#cic-payment](https://nikedigital.slack.com/messages/C0Z9P2E5Q)|
|Confluence Space|[CiC Payment](https://confluence.nike.com/display/PHYLON/Payment+Team+Playbook)|
