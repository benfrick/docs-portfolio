...
function lunr_search(term) {
    ...
    if(term) {
        document.getElementById('modtit').innerHTML = "<h2 class='modal-title'>Search results for '" + term + "'</h2>" + document.getElementById('modtit').innerHTML;
        //put results on the screen.
        var results = idx.search(term);
        if(results.length>0){
            //console.log(idx.search(term));
            ...
            document.querySelectorAll('#lunrsearchresults ul')[0].innerHTML = "<li class='lunrsearchresult'>Sorry, no results found. Close and try a different search.</li>";
        }
    }
    return false;
}
